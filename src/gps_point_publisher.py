...
class AutomowerGpsPublisher(object):
	...
	def publish_gps_file(self):
		...
		for i in range(len(self.times)):
			...
			if time_diff >= 0:
				# As long as positive time_diff set new previous points time
	...
	def read_gpspipe(self):
		...
		if not line:
			...
			print(sentence)
			if sentence['class'] == 'TPV' and sentence['mode'] == 3:
...
if __name__ == '__main__':
	if len(sys.argv) > 1:
		...
		if arg == 0 or arg == 1:
			...
			slam_gps = AutomowerGpsPublisher(arg)
			slam_gps.run()
		else:
